...
NBServer::NBServer(uint16_t port, bool synch) :
  _port(port),
  _synch(synch),
  _socket(-1),
...
NBClient NBServer::available(bool synch)
{
  MODEM.poll();
  ...
  return NBClient(socket, synch);
}

void NBServer::beginWrite()
{
}

size_t NBServer::write(uint8_t c)
{
  return write(&c, sizeof(c));
}

size_t NBServer::write(const uint8_t *buf)
{
  return write(buf, strlen((const char*)buf));
}

size_t NBServer::write(const uint8_t *buf, size_t sz)
{
  size_t written = 0;
  ...
  if (_socket != -1) {
    for (int i = 0; i < MAX_CHILD_SOCKETS; i++) {
      if (_childSockets[i].socket != -1) {
        NBClient client(_childSockets[i].socket, true);
...
void NBServer::handleUrc(const String& urc)
{
